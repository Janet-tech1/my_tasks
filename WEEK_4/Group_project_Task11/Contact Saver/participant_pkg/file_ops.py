...
# Converts the string path into a Path object.
    file_janet = Path(path)

# A header if it doesn’t exist)
    try:
        # Check if file exists
        file_janet_exists = file_janet.exists()

        # Open in append mode
        # Write data to CSV file
        with open(file_janet, "a", newline="", encoding="utf-8") as janet_csv:
            writer = csv.writer(janet_csv)

            # Write header if file is new
            if not file_janet_exists:
            ...
            # participant_dict is a dictionary holding participant details, e.g.: {"name": "Janet", "age": 99, "phone": "08133274250", "track": "AI Developer"}
            writer.writerow([
